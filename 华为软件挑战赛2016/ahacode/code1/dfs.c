...
修改时间：20170219
*/
void dfs_process(int step, int *book,int *pDataIn, int *pOutputNum, int *pDataOut)
{
	int i = 0;
	if(step == Num)
	{
	...
	for(i = 0; i < Num; i++)
	{
		if(*(book + i) == 0)
		{
			...
			*(book + i) = 1;
			dfs_process(step+1, book, pDataIn, pOutputNum, pDataOut);//递归调用，实现深度优先搜索
			*(book + i) = 0;
		}
	}
	return;
...
int dfs_main(int *dataIn)
{
	...
	dfs_process(step, book, pDataIn, &outputNum, pDataOut);//调用深度优先算法处理算法
	printf("\n total = %d\n",outputNum);
...
int main()
{
	...
	ret = dfs_main(dataIn);//调用深度优先算法处理函数入口
	
	if(ret == VOS_TRUE)
	{
		printf("[Info]dfs run success\n");
	}
	else
	{
		printf("[Error]dfs run fail!\n");
	}
